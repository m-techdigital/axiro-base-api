...
class MarketplaceDocumentService
{
    ...
    public function ensureForTransaction(Transaction $transaction): Collection
    {
        ...
        if ($transaction->transaction_type === 'rental' && in_array($transaction->status, ['return_pending','returned','completed','disputed'], true)) $types[] = 'return_minutes';
        if ($transaction->status === 'disputed' || $transaction->disputes->isNotEmpty()) $types[] = 'dispute_minutes';
        if ($transaction->disputes->contains(fn ($d) => $d->status === 'resolved')) $types[] = 'dispute_resolution';
    ...
    {
        if (!isset(self::TYPES[$type])) throw ValidationException::withMessages(['document_type' => 'Loại tài liệu không hợp lệ.']);
        ...
        if ($existing && !$regenerate) return $existing->load(['template','acceptances.customer:id,code,name']);
        $version = $existing ? $existing->version + 1 : 1;
        $payload = $this->payload($transaction->fresh()->load(['product','listing','buyer','seller','contract','payments','events','disputes','checkpoints']));
        $html = $this->render($template->content_html, $payload);
        if (preg_match('/\{\{[^}]+\}\}/', $html, $matches)) throw ValidationException::withMessages(['content_html' => 'Tài liệu còn trường trộn chưa được thay thế: '.$matches[0]]);
    ...
    public function accept(GeneratedDocument $document, int $customerId, Request $request): DocumentAcceptance
    {
        ...
        if (!in_array($customerId, [$transaction->buyer_customer_id, $transaction->seller_customer_id], true)) abort(403);
        $existing = DocumentAcceptance::query()->where('generated_document_id', $document->id)->where('customer_id', $customerId)->first();
        if ($existing?->status === 'accepted') return $existing->load('customer:id,code,name');
        ...
        if ($document->acceptances()->count() >= 2) $document->update(['status' => 'accepted']);
        return $acceptance;
    }
    ...
    public function pdf(GeneratedDocument $document): string
    {
        ...
        $dompdf = new Dompdf($options); $dompdf->loadHtml($document->rendered_html, 'UTF-8'); $dompdf->setPaper('A4', 'portrait'); $dompdf->render();
    ...
    public function payload(Transaction $transaction): array
    {
        ...
        if ($transaction->payments->isEmpty()) $payments = '<div class="notice">Chưa có kế hoạch thanh toán.</div>';
        ...
        return [
            ...
            'transaction_code' => $transaction->code, 'transaction_type' => $transaction->transaction_type === 'rental' ? 'Thuê tài khoản' : 'Mua bán tài khoản',
            'purchase_mode' => match ($transaction->purchase_mode) {'installment' => 'Trả góp', 'deposit' => 'Đặt cọc', default => 'Thanh toán đủ'},
            'listing_code' => $transaction->listing?->code ?: '—', 'listing_title' => $transaction->listing?->title ?: '—',
            'listing_type' => $transaction->listing?->listing_type === 'rental' ? 'Cho thuê' : 'Bán',
            'product_name' => $transaction->product?->name ?: 'Tài khoản trò chơi', 'product_code' => $transaction->product?->code ?: '—',
            'game_code' => $transaction->product?->game_code ?: $transaction->product?->product_type ?: '—', 'server_name' => $transaction->product?->server_name ?: '—',
    ...
    private function money(mixed $value): string { return number_format((float)$value, 0, ',', '.').' đ'; }
    private function label(string $value): string { return match ($value) {'pending'=>'Đang chờ','submitted'=>'Đã gửi đối soát','confirmed'=>'Đã xác nhận','rejected'=>'Bị từ chối','pending_payment'=>'Chờ thanh toán','partially_paid'=>'Đã thanh toán một phần','paid'=>'Đã thanh toán','handover_pending'=>'Chờ bên nhận xác nhận','handed_over'=>'Đã bàn giao','active'=>'Đang thuê','return_pending'=>'Chờ xác nhận hoàn trả','returned'=>'Đã hoàn trả','completed'=>'Hoàn tất','cancelled'=>'Đã hủy','disputed'=>'Đang tranh chấp',default=>$value}; }
}
