...
class WalletLedgerService {
    ...
    public function settleHeldWithFee(int $customerId,string $grossAmount,string $sellerNetAmount,array $context=[]): array {
        return DB::transaction(function()use($customerId,$grossAmount,$sellerNetAmount,$context){
            if(bccomp($sellerNetAmount,$grossAmount,2)>0)throw ValidationException::withMessages(['settlement'=>'Số tiền ròng không được vượt số tiền tạm giữ.']);
            $base=(string)($context['idempotency_key']??Str::uuid());
            ...
            $in=null;
            if(bccomp($sellerNetAmount,'0.00',2)>0)$in=$this->creditAvailable($customerId,$sellerNetAmount,'settlement_net_credit',array_merge($context,['idempotency_key'=>$base.':net','internal_transfer'=>true]));
    ...
    public function mutate(int $customerId,string $amount,string $direction,string $bucket,string $type,array $context=[]): WalletTransaction {
        if(bccomp($amount,'0.00',2)<=0)throw ValidationException::withMessages(['amount'=>'Số tiền phải lớn hơn 0.']);
        return DB::transaction(function()use($customerId,$amount,$direction,$bucket,$type,$context){
            ...
            if($bucket==='available'){
                if($sign<0 && bccomp($availableBefore,$amount,2)<0)throw ValidationException::withMessages(['wallet'=>'Số dư khả dụng không đủ.']);
                $availableAfter=$sign>0?bcadd($availableBefore,$amount,2):bcsub($availableBefore,$amount,2);$heldAfter=$heldBefore;
            }else{
                if($sign<0 && bccomp($heldBefore,$amount,2)<0)throw ValidationException::withMessages(['wallet'=>'Số dư đang tạm giữ không đủ.']);
                $heldAfter=$sign>0?bcadd($heldBefore,$amount,2):bcsub($heldBefore,$amount,2);$availableAfter=$availableBefore;
            }
            $wallet->update([
