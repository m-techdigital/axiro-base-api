...
final class MarketplaceRiskService
{
    ...
    public function evaluateWithdrawal(int $withdrawalId, string $amount, int $customerId): void
    {
        ...
        if (bccomp($amount, $threshold, 2) >= 0) {
            $this->flag('withdrawal_request', $withdrawalId, 'large_withdrawal', 'high', 'Yêu cầu rút tiền vượt ngưỡng cần kiểm tra thủ công.', ['amount' => $amount, 'customer_id' => $customerId, 'threshold' => $threshold]);
        }
    }
}
