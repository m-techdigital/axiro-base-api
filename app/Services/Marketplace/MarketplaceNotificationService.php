...
class MarketplaceNotificationService
{
    public function send(int $customerId, string $type, string $title, string $message, ?string $actionUrl = null, array $data = []): ?MarketplaceNotification
    {
        ...
        if ($preference && ! $preference->in_app && $category !== 'security') {
            return null;
        }
    ...
    {
        if (str_contains($type, 'payment') || str_contains($type, 'deposit') || str_contains($type, 'withdrawal')) {
            return 'payment';
        }
        if (str_contains($type, 'handover') || str_contains($type, 'return')) {
            return 'handover';
        }
        if (str_contains($type, 'rental') || str_contains($type, 'due')) {
            return 'rental_due';
        }
        if (str_contains($type, 'document') || str_contains($type, 'contract')) {
            return 'document';
        }
        if (str_contains($type, 'case') || str_contains($type, 'dispute')) {
            return 'case';
        }
