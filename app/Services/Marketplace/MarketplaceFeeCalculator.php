...
final class MarketplaceFeeCalculator {
 public function calculate(string $transactionType,string $value):array{
  ...
  $buyerRate=(string)($policy?->buyer_fee_rate??0);$sellerRate=(string)($policy?->seller_fee_rate??0);$taxRate=(string)($policy?->tax_rate??0);
  ...
  $platform=bcadd($buyer,$seller,2);$sellerNet=bcsub($value,$seller,2);if(bccomp($sellerNet,'0.00',2)<0)$sellerNet='0.00';
