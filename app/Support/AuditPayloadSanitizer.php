...
final class AuditPayloadSanitizer
{
    ...
    {
        if ($depth > 8) return '[Đã rút gọn do dữ liệu lồng quá sâu]';
        ...
        if (is_object($value)) return self::sanitize((array) $value, $depth + 1);
        if (is_string($value) && mb_strlen($value) > 3000) return mb_substr($value, 0, 3000).'… [đã rút gọn]';
        return $value;
    }
    ...
    private static function isSensitive(string $key): bool
    {
        foreach (self::SENSITIVE_KEYS as $sensitive) {
            if ($key === $sensitive || str_contains($key, $sensitive)) return true;
        }
        return false;
    }
}
