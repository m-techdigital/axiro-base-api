...
class CustomerSecurityController extends Controller {
    ...
    public function beginTwoFactorSetup(){ return success_response($this->twoFactor->beginSetup(auth('customer_api')->user()),'Đã tạo khóa xác thực hai lớp.'); }
    public function confirmTwoFactor(Request $request){ $data=$request->validate(['code'=>'required|string|max:20']); return success_response($this->twoFactor->confirm(auth('customer_api')->user(),$data['code']),'Đã bật xác thực hai lớp.'); }
    public function regenerateRecoveryCodes(Request $request){ $data=$request->validate(['code'=>'required|string|max:20']); return success_response($this->twoFactor->regenerateRecoveryCodes(auth('customer_api')->user(),$data['code']),'Đã tạo lại mã khôi phục.'); }
    public function disableTwoFactor(Request $request){ $data=$request->validate(['password'=>'required|string','code'=>'required|string|max:20']); $this->twoFactor->disable(auth('customer_api')->user(),$data['password'],$data['code']); return success_response(null,'Đã tắt xác thực hai lớp.'); }
