...
class CustomerTransactionExperienceTest extends TestCase
{
    ...
    public function test_customer_transaction_does_not_offer_cancel_and_keeps_product_details(): void
    {
        ...
        $product = Product::create(['code' => 'PRD-001', 'name' => 'Nhân vật Ninja School cấp 110', 'product_type' => 'game_account', 'game_code' => 'ninja_school', 'server_name' => 'Server 1', 'level' => '110', 'status' => 'active', 'approval_status' => 'approved', 'is_published' => true, 'availability_status' => 'available', 'sale_price' => 100000, 'published_at' => now(), 'attributes' => ['class' => 'Kunai'], 'owner_customer_id' => $seller->id]);
        $transaction = Transaction::create(['code' => 'TRX-001', 'transaction_type' => 'purchase', 'purchase_mode' => 'full',
            'product_id' => $product->id, 'buyer_customer_id' => $buyer->id, 'seller_customer_id' => $seller->id, 'transaction_value' => 100000, 'total_payable' => 100000, 'paid_amount' => 0, 'transaction_date' => now()->toDateString(), 'status' => 'pending_payment']);
