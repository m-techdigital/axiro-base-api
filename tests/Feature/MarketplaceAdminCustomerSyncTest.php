...
class MarketplaceAdminCustomerSyncTest extends TestCase {
    ...
    private function adminHeaders(): array { $admin=User::factory()->create(['username'=>'admin-sync']); return ['Authorization'=>'Bearer '.auth('api')->login($admin)]; }
    public function test_listing_payment_handover_and_notifications_are_synchronized(): void {
        $buyer=Customer::factory()->create();$seller=Customer::factory()->create();
        $product=Product::create(['code'=>'SYNC-001','name'=>'Tài khoản đồng bộ','product_type'=>'game_account','status'=>'active','price'=>500000,'owner_customer_id'=>$seller->id]);
        $listing=ProductListing::create(['code'=>'LST-SYNC-001','product_id'=>$product->id,'owner_customer_id'=>$seller->id,'listing_type'=>'sale','status'=>'published','title'=>'Tin đồng bộ','sale_price'=>500000,'published_at'=>now()]);
        $transaction=$this->postJson('/api/v1/customer/listings/'.$listing->id.'/transact',[], $this->customerHeaders($buyer))->assertCreated()->json('data');
