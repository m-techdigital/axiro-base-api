...
use App\Models\{Customer,CustomerWallet,Product,ProductListing,User};
...
class MarketplaceFinanceClosureTest extends TestCase {
 use RefreshDatabase;
 private function customerHeaders(Customer $customer):array{return ['Authorization'=>'Bearer '.auth('customer_api')->login($customer)];}
 private function adminHeaders():array{$admin=User::factory()->create(['username'=>'admin-finance']);return ['Authorization'=>'Bearer '.auth('api')->login($admin)];}
 public function test_wallet_payment_records_before_after_and_releases_to_seller_on_completion():void{
  $buyer=Customer::factory()->create();$seller=Customer::factory()->create();CustomerWallet::create(['customer_id'=>$buyer->id,'available_balance'=>1000000]);CustomerWallet::create(['customer_id'=>$seller->id]);
  $product=Product::create(['code'=>'FIN-001','name'=>'Tài khoản tài chính','product_type'=>'game_account','status'=>'active','price'=>500000,'owner_customer_id'=>$seller->id]);
  $listing=ProductListing::create(['code'=>'LST-FIN-001','product_id'=>$product->id,'owner_customer_id'=>$seller->id,'listing_type'=>'sale','status'=>'published','title'=>'Tài khoản tài chính','sale_price'=>500000,'published_at'=>now()]);
  $transaction=$this->postJson('/api/v1/customer/listings/'.$listing->id.'/transact',['payment_method'=>'wallet'],$this->customerHeaders($buyer))->assertCreated()->json('data');
 ...
 public function test_rental_rate_creates_deposit_and_periodic_cycles_then_refunds_deposit():void{
  $renter=Customer::factory()->create();$lessor=Customer::factory()->create();CustomerWallet::create(['customer_id'=>$renter->id,'available_balance'=>2000000]);CustomerWallet::create(['customer_id'=>$lessor->id]);
  $product=Product::create(['code'=>'RENT-001','name'=>'Tài khoản cho thuê','product_type'=>'game_account','status'=>'active','price'=>0,'owner_customer_id'=>$lessor->id]);
  $listing=ProductListing::create(['code'=>'LST-RENT-001','product_id'=>$product->id,'owner_customer_id'=>$lessor->id,'listing_type'=>'rental','status'=>'published','title'=>'Thuê 3 ngày','rental_price'=>300000,'rental_price_unit'=>'day','rental_period_unit'=>'day','minimum_rental_period'=>1,'rental_billing_mode'=>'periodic','rental_billing_cycle_unit'=>'day','rental_billing_cycle_count'=>1,'deposit_amount'=>400000,'published_at'=>now()]);
  ...
  $transaction=$this->postJson('/api/v1/customer/listings/'.$listing->id.'/transact',['rental_rate_id'=>$rate->id,'payment_method'=>'wallet','rental_start_at'=>now()->toISOString()],$this->customerHeaders($renter))->assertCreated()->assertJsonCount(4,'data.payments')->json('data');
  $this->assertEquals('1150000.00',$transaction['total_payable']);
  foreach($transaction['payments'] as $payment){$this->postJson('/api/v1/customer/transactions/'.$transaction['id'].'/payments/'.$payment['id'].'/submit',['payment_method'=>'wallet'],$this->customerHeaders($renter))->assertOk();}
