...
class CustomerSecurityWorkflowTest extends TestCase {
 use RefreshDatabase;
 private function headers(Customer $customer): array { return ['Authorization'=>'Bearer '.auth('customer_api')->login($customer)]; }
 public function test_profile_email_cannot_be_changed_directly_and_requires_token(): void {
  Mail::fake();$customer=Customer::factory()->create(['email'=>'old@example.com']);
