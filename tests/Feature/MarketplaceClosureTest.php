...
use App\Models\{Customer,MarketplaceDispute,MarketplaceFeePolicy,Product,ProductListing,Transaction,TransactionAssetSnapshot};
...
class MarketplaceClosureTest extends TestCase {
 ...
 public function test_case_and_asset_snapshot_share_transaction_owner(): void {
  ...
  $product=Product::create(['code'=>'PRO-1','name'=>'Nick test','product_type'=>'ninja_school','status'=>'active']);
  $listing=ProductListing::create(['code'=>'LST-1','product_id'=>$product->id,'owner_customer_id'=>$seller->id,'listing_type'=>'rental','title'=>'Nick test','status'=>'published','rental_price'=>100000,'deposit_amount'=>50000]);
  $transaction=Transaction::create(['code'=>'TRX-1','transaction_type'=>'rental','purchase_mode'=>'rental','listing_id'=>$listing->id,'product_id'=>$product->id,'buyer_customer_id'=>$buyer->id,'seller_customer_id'=>$seller->id,'transaction_value'=>100000,'total_payable'=>150000,'seller_net_amount'=>100000,'transaction_date'=>now()->toDateString(),'status'=>'paid']);
